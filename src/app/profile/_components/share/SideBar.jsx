"use client";
import { Person } from "../../../../assets/profile/user.png";
    ...
    <div className="md:block hidden lg:w-[300px] md:w-[220px] pb-16 border-r-[2px]">
      <div>
        ...
        {/* <div className="flex flex-col gap-2 justify-center items-center my-4">
          ...
          <CustomBtn
            style={`w-[200px] ${
              path === "/dashboard/profile" && "bg-[#FF6702]/40"
            }`}
            title={` Profile`}
            icon={<User />}
            click={() => (window.location.href = "/dashboard/profile")}
          />
        ...
        <ul className="">
          {sidebarList.map((item) => (
            <li
              key={item.id}
              className={`hover:bg-gray-200 hover:text-primary hover:font-bold py-3 lg:px-12 px-5 ${
